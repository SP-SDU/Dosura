...
class MedicationManagementTest {
    ...
    fun testAdherenceCalculation() = runBlocking {
        ...
        val stats = logRepository.calculateAdherenceStats(userId, 7)
        assertEquals(7, stats.totalScheduled)
        assertEquals(5, stats.totalTaken)
        assertEquals(2, stats.totalMissed)
        assertTrue(stats.adherencePercentage > 70f)
        assertTrue(stats.adherencePercentage < 72f)
    }
